...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	//icp.setRANSACOutlierRejectionThreshold (10);
  	...
  	if (icp.hasConverged ())
  	{
  		std::cout << "\nICP has converged with a score of " << icp.getFitnessScore () << std::endl;
...
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	pcl::console::TicToc time;
  	time.tic();
  	//Implement the PCL NDT function and return the correct transformation matrix
  	Eigen::Matrix4f init_guess = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch,
                                             startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z).cast<float>();
  	// settin max number of registration iterations.
  	ndt.setMaximumIterations(iterations);
  	ndt.setInputSource(source);
  	
  	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ndt (new pcl::PointCloud<pcl::PointXYZ>);
  	ndt.align (*cloud_ndt, init_guess);
  	cout << "Normal Distributions Transform has converged:" << ndt.hasConverged () << "with a score of: " << ndt.getFitnessScore () <<  " time: " << time.toc() <<  " ms" << endl;
...
int main(int argc, char *argv[]){
	...
	int scans = 0; //define the beginning.
  	int ndt_option = 1;
  	if (argc == 2 && strcmp(argv[1], "2") == 0)
      ndt_option = 0;
  	printf("\n%s\n\n", ndt_option ? "Using Normal Distributions Transform (NDT)" : "Using Iterative Closest Point (ICP)");
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			new_scan = true;
          	// ground truch is used at the beginning.
          	if(scans == 0) {
			...
			//pose = ....
			Eigen::Matrix4d transform = ndt_option ? NDT(mapCloud, cloudFiltered, pose, 5) : ICP(mapCloud, cloudFiltered, pose, 10);
          	//note: icp iterations, tried 3 / 
